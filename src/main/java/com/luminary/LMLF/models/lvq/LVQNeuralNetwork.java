...
import com.luminary.LMLF.utils.SMath;
...
public class LVQNeuralNetwork {
    ...
    private final double step_dec_rate;    // step decrease rate, recommended: 0.99
    private final double min_step_size;    // minimum step size, recommended: 0.10
    ...
    private TreeMap<Double, Integer> getDistanceToClassCenters(double[] vector) {
        ...
        for (int i = 0; i <= classCenters.size() - 1; i++)
            distanceToInput.put(SMath.euclideanDistance(vector, classCenters.get(i).getData()), i);
    ...
    public void normalize() {
        minMaxOfRow = SMath.normalize(vectors);
    }
    ...
    public void train() {
        ...
        for (LabeledData vector : vectors) {
            ...
            double[] distToNearestOutput = SMath.multiply(SMath.subtract(vector.getData(), nearestOutput.getData()), step_size);
            ...
            if (vector.getCategory() == nearestOutput.getCategory())
                nearestOutput.setData(SMath.add(nearestOutput.getData(), distToNearestOutput));
            else
                nearestOutput.setData(SMath.subtract(nearestOutput.getData(), distToNearestOutput));
        }
    ...
    public LVQNeuralNetworkPredictResult predict(double[] vector) {
        ...
        for (int i = 0; i <= vector.length - 1; i++)
            vectorNormalized[i] = SMath.normalize(vector[i], minMaxOfRow[i][0], minMaxOfRow[i][1]);
    ...
    void printVectors() {
        ...
        // draw
        System.out.println("+" + ("--".repeat(vectors.size())) + "+");
